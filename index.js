(async function () {
    ...
    try {
        ...
        function render() {
            ...
            const gameEl = panel.querySelector('[data-role="game"]');
...
#ie-panel{position:fixed;z-index:999989;width:min(430px,calc(100vw - 18px));max-height:min(82vh,720px);display:none;color:#f5f1e8;font-family:"Noto Sans SC","Microsoft YaHei",sans-serif;filter:drop-shadow(0 24px 50px rgba(0,0,0,.5))}
...
`;
            ...
            panel.id = 'ie-panel';
            panel.innerHTML = `<div class="ie-card"><div class="ie-head"><div class="ie-mark">∞</div><div><div class="ie-title">无限流状态与赌场</div><div class="ie-sub">程序账本 · 正文同步 · 聊天独立</div></div><button class="ie-close" data-role="close" type="button" aria-label="关闭">×</button></div><div class="ie-body"><div class="ie-grid"><div class="ie-stat"><div class="ie-label">可用积分</div><div class="ie-value gold" data-role="score">0</div></div><div class="ie-stat"><div class="ie-label">冻结积分</div><div class="ie-value" data-role="frozen">0</div></div><div class="ie-stat"><div class="ie-label">总资产</div><div class="ie-value" data-role="asset">0</div></div></div><div class="ie-section">当前剧情</div><div class="ie-info"><div class="ie-line"><span class="ie-label">地点</span><b data-role="location">回廊</b></div><div class="ie-line"><span class="ie-label">时间</span><b data-role="time">无</b></div><div class="ie-line"><span class="ie-label">等级 / 位格</span><b><span data-role="level">D</span> / <span data-role="rank">未知</span></b></div><div class="ie-line"><span class="ie-label">任务</span><b data-role="task">休整期</b></div><div class="ie-line"><span class="ie-label">待清算</span><b data-role="pending">无</b></div><div class="ie-line"><span class="ie-label">在场</span><b data-role="presence">无</b></div></div><div class="ie-section">道具库存</div><div class="ie-chips" data-role="items"></div><div class="ie-section">快速下注 · 选择游戏</div><div class="ie-hint" style="margin:0 0 7px">当前赌局：<span data-role="game">无</span></div><div class="ie-games" data-role="games"><button type="button" class="ie-game-btn active" data-game="dice-high">骰子·比大小</button><button type="button" class="ie-game-btn" data-game="odd-even">骰子·猜单双</button><button type="button" class="ie-game-btn" data-game="slots">老虎机</button><button type="button" class="ie-game-btn" data-game="blackjack">简易21点</button></div><div class="ie-form" style="margin-top:7px"><input data-role="bet" type="number" min="1" step="1" placeholder="下注积分" class="wide"><button class="wide" data-role="play" type="button">下注并立即开局</button></div><div class="ie-section">托管交易</div><div class="ie-form"><select class="wide" data-role="trade-direction"><option value="in">收入 / 对方支付</option><option value="out">支出 / 支付对方</option></select><input data-role="trade-amount" type="number" min="1" step="1" placeholder="交易金额" class="wide"><input class="wide" data-role="trade-reason" type="text" placeholder="交易说明，例如：购买治疗针"><button class="wide" data-role="trade" type="button">确认托管结算</button></div><div class="ie-actions"><button data-role="settle" type="button">查看当前赌局</button><button data-role="export" type="button">导出账本</button></div><div class="ie-section">最近流水</div><div class="ie-ledger" data-role="ledger"></div><div class="ie-warning" data-role="warning" hidden></div><div class="ie-hint">正文中出现“输了300积分 / 获得500积分 / 积分：700（↓300）”时，下一次刷新会自动记入账本。状态栏中的积分仅作核对，账本优先。</div></div></div>`;
        ...
        let selectedGame = 'dice-high';
        ui.panel.querySelectorAll('[data-role="games"] .ie-game-btn').forEach((button) => {
            addListener(button, 'click', () => {
                selectedGame = button.dataset.game;
                ui.panel.querySelectorAll('[data-role="games"] .ie-game-btn').forEach((other) => other.classList.toggle('active', other === button));
            });
        });
